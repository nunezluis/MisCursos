...
\begin{itemize}
	...
	\item<2-> Proyectamos la ecuación $0=\dot{\vec{R}}+  \vec{\Omega} \times \vec{r}(cp)$ al sistema $S$, (${\bf i}$, ${\bf j}$, ${\bf k}$) tendremos 
...
\begin{itemize}
	\item<1-> La energía cinética será $\left.T=\frac{1}{2} M\left(\dot{x}^2+\dot{y}^2+\dot{z}^2\right)+\frac{1}{2}\left[I_{1}(c)\left(\Omega_{1}\right)^2+I_{2}(c)\left(\Omega_{2}\right)^2+I_{3}(c) \Omega_{3}\right)^2\right]$.  
...
\begin{itemize}  
	\item<1-> En principio tenemos seis grados de libertad $(x, y, z, \phi, \psi, \theta)$
...
\begin{itemize}
	...
	\item<3-> Usando las otras ecuaciones de ligadura $\dot{x}=b \dot{\phi} \cos \phi \operatorname{sen} \theta, \quad \dot{y}=b \dot{\phi} \operatorname{sen} \phi \operatorname{sen} \theta, \quad \dot{z}=0$, obtenemos $\dot{\psi}+\dot{\phi}\left[\frac{b}{a} \operatorname{sen} \theta+\cos \theta\right]=0 \Rightarrow \psi=-\frac{\sqrt{a^2+b^2}}{a} \phi$
