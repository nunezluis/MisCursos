...
%%%%% Diapo 1
\section{Hamiltoniano y Péndulo}
\subsection{El problema y las coordenadas}
\frame{
  \frametitle{Hamiltoniano y Péndulo}
El punto de suspensión de un péndulo está obligado a moverse a lo largo de la parábola $y=a x^2$. Encontrar el Hamiltoniano.
  \begin{figure}[t]
	\includegraphics[width=1.8in]{Figuras/PenduloCurva.png}
   \end{figure}
   \begin{itemize}  
  	\item<1-> Sen las coordenadas del punto de sustentación del péndulo son $x, y$ y las coordenadas de la masa $x^{\prime}, y^{\prime}$, 
...
\begin{itemize}  
	\item<1-> La energía cinética viene dada por: 
	...
	\item<2-> y la energía potencial por: $V=m g y^{\prime}=m g\left(a x^2-l \cos \theta\right)$
...
%
%%%%% Diapo 2
\section{$\mathcal{H}=q+t e^p$ y la transformación $Q=q+e^{p}, P=p$}
\frame{
\frametitle{$\mathcal{H}=q+t e^p$ y $Q=q+e^p \quad \& \quad P=p$ 1/2}
El hamiltoniano de un cierto sistema físico es: $\mathcal{H}=q+t e^p$. Muestre que la transformación $Q=q+e^p, P=p$ es una transformación canónica. Seguidamente, encuentre la función generatriz de esta transformación. Finalmente, determine el nuevo hamiltoniano y resuelva las ecuaciones de movimiento en las nuevas coordenadas.\\ 
$\cdot$ \\
Para determinar si la transformación es canónica, construimos el paréntesis de Poisson entre las coordenadas
...
\begin{itemize}  
	...
	\item<4-> Por lo tanto, como la tranformación cumple con 
...
Para encontrar la función generadora $ F $ supondremos una función generadora del tipo $F_2(q, P)$, que depende de  $q$ y  $P$
\begin{itemize}  
	...
	\item<3->  La función generadora es $F_2(q, P) = qP + e^P$
\end{itemize}
}
%
%%%%% Diapo 2
...
\begin{itemize}  
	\item<1-> El Lagrangeano será $\mathcal{L} =\dot{q} p-\mathcal{H}$. De la ecuación de movimiento 
...
\begin{itemize}  
	\item<1-> $f\left(q_1, p_1\right)$ es una constante del movimiento 
	 ...
	 \item<3-> $\phi$ es una coordenada cíclica y $p_\phi$ es una constante del movimiento.
...
Mostrar que la transformación: $q=Q \cos \gamma t+\frac{P}{m \omega} \operatorname{sen} \gamma t$, $p=-m \omega Q \operatorname{sen} \gamma t+P \cos \gamma t$ es canónica. Determinar la función $\mathcal{F}_2 = \mathcal{F}_2\left(q_i, P_i, t\right)$.  Aplicar esta transformación al oscilador armónico.
\begin{itemize}  
	\item<1-> Si $q = q(Q, P, t)$ y $p = p(Q, P, t)$ es canónica entonces $\{q, p\}=\frac{\partial q}{\partial Q} \frac{\partial p}{\partial P}-\frac{\partial q}{\partial P} \frac{\partial p}{\partial Q}=\cos ^2 \gamma t+\operatorname{sen}^2 \gamma t=1$ $\checkmark$
	\item<2-> Claramente $Q=\frac{q}{\cos \gamma t}-\frac{P}{m \omega} \tan \gamma t\, , \quad$ y  $\quad p=\frac{P}{\cos \gamma t}-q m \omega \tan \gamma t$
	\item<3-> Por lo tanto, ara encontrar la función generadora $\mathcal{F}_2 = \mathcal{F}_2\left(q_i, P_i, t\right)$, integramos las ecuaciones $\frac{\partial \mathcal{F}_2}{\partial q}  =p=\frac{P}{\cos \gamma t}-q m \omega \tan \gamma t$, y $\frac{\partial \mathcal{F}_2}{\partial P} =Q=\frac{q}{\cos \gamma t}-\frac{P}{m \omega} \tan \gamma t$
	\item<4-> La solución es $\mathcal{F}_2(q, P, t)=\frac{q P}{\cos \gamma t}-\frac{1}{2} q^2 m \omega \tan \gamma t-\frac{1}{2} \frac{P^2}{m \omega} \tan \gamma t$
